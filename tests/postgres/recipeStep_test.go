...
func Test_recipeStepRepository_Create(t *testing.T) {
	repo := postgres.NewRecipeStepRepository(testDbInstance)
	...
	tests := []struct {
		...
		}, // успешное создание
		{
			...
			wantErr: true,
			errStr: errors.New("creating recipe step: ERROR: insert or update on table \"recipestep\" " +
				"violates foreign key constraint \"recipestep_recipeid_fkey\" (SQLSTATE 23503)"),
...
func Test_recipeStepRepository_GetById(t *testing.T) {
	...
	tests := []struct {
		...
		}, // успешное получение
		{
			...
			id:      uuid.Nil,
			wantErr: true,
			errStr:  errors.New("getting recipe step by id: no rows in result set"),
		}, // отсутствие id
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
